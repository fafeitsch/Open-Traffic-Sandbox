...
func TestInit(t *testing.T) {
	...
	assert.Equal(t, 10, len(assignment.WayPoints), "number of waypoints in the line")
	assert.Equal(t, WayPoint{Departure: 24000000, IsRealStop: true, Name: "Mainfranken Theater", Latitude: 49.7947186, Longitude: 9.9359725}, assignment.WayPoints[2], "sample waypoint")

	assert.Equal(t, assignment.Name, line.Name, "name of the line")
	assert.Equal(t, 10, len(line.Stops), "number of stops in the line")
	assert.Equal(t, 39, len(line.departures[line.Stops[9].id]), "number of tours in the line")
