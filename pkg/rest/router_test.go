...
func TestNewRouter(t *testing.T) {
	...
	router := NewRouter(mdl, func(coordinate ...model.Coordinate) ([]model.Coordinate, float64, error) {
	...
	t.Run("get lines", func(t *testing.T) {
		...
		assert.Equal(t, model.LineId("A-inbound"), lines[1].Key, "line key")
	})
	t.Run("get line", func(t *testing.T) {
		...
		assert.Equal(t, model.LineId("A-inbound"), line.Key, "line key")
	})
